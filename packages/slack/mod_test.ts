...
Deno.test("sendMessage posts through Slack chat.postMessage", async () => {
  await withFetch(async (_input, init) => {
    ...
    assertEquals(JSON.parse(String(init?.body)), { channel: "C123", text: "hello" });
    return Response.json({ ok: true, channel: "C123", ts: "123.456" });
  }, async () => {
    const result = await sendMessage({ token: "xoxb-test", channel: "C123", text: "hello" }).run(event, context);
...
Deno.test("sendMessage treats Slack API errors as failures", async () => {
  await withFetch(() => Promise.resolve(Response.json({ ok: false, error: "not_in_channel" })), async () => {
    const result = await sendMessage({ token: "xoxb-test", channel: "C123", text: "hello" }).run(event, context);
